...
def main() -> None:
    ...
    with content_col:
        ...
        if page == "Setup master":
            ...
            source = _load_ownership_data()
            filtered = _filter_raci(source, selected_role, selected_area)
            _render_raci_setup_page(filtered, roles, areas, selected_role)
        elif page == "Timeline":
            source = _load_ownership_data()
...
def _read_sql_query(sql: str, conn: Any, params: tuple[Any, ...] = ()) -> pd.DataFrame:
    try:
        ...
        if not _using_postgres():
            raise
        _postgres_connection.clear()
        with _connect() as retry_conn:
            return pd.read_sql_query(sql, retry_conn, params=params)
...
def _rename_columns(df: pd.DataFrame, columns: dict[str, str]) -> pd.DataFrame:
    normalized = {column.lower(): target for column, target in columns.items()}
    return df.rename(columns={column: normalized.get(column.lower(), column) for column in df.columns})
...
def _render_raci_setup_page(
    ...
    _render_ownership_editor(df, roles, areas, selected_role)
...
def _load_finance_modules() -> list[dict[str, str]]:
    ...
    rows = _rename_columns(rows, {"name": "Name", "status": "Status", "description": "Description"})
    if rows.empty:
...
def _load_master_items(table: str, defaults: list[str]) -> list[str]:
    with _connect() as conn:
        rows = _read_sql_query(f"SELECT name FROM {table} ORDER BY sort_order, name", conn)
    if rows.empty:
        return defaults
    return rows["name"].astype(str).tolist()
...
def _selected_role_matches(row: pd.Series, selected_role: str) -> str:
    matches = []
    for column in ["Responsible", "Accountable", "Consulted", "Informed"]:
        if selected_role in _split_roles(row[column]):
...
def _render_ownership_editor(
    ...
    if selected_role != "All roles":
        display.insert(
            2,
            "Selected role match",
            display.apply(lambda row: _selected_role_matches(row, selected_role), axis=1),
        )
    ...
    disabled_columns = []
    if selected_role != "All roles":
        column_config["Selected role match"] = st.column_config.TextColumn(
            "Selected role match",
            help="Shows where the selected role appears in this RACI row.",
        )
        disabled_columns.append("Selected role match")
...
def _load_timing_for_period(year: int, month: int) -> pd.DataFrame:
    ...
    overrides = _rename_columns(
        overrides,
        {"activity": "Activity", "start": "Start", "end": "End", "status": "Status"},
    )
